...
thinning {Δ = Δ , y ∷ b} x∉ΓΔy Γ⊢b (pi-intro rule ΓΔ⊢a ΓΔyx⊢b) =
  pi-intro rule
    (thinning x∉ΓΔy Γ⊢b ΓΔ⊢a)
    λ {z} z∉ΓxΔy → (thinning {!!} Γ⊢b (ΓΔyx⊢b {!!}))
thinning {Δ = Δ , y ∷ b} x∉ΓΔ Γ⊢b (abstr x ΓΔ⊢m x₁ x₂) = {!!}
thinning {Δ = Δ , y ∷ b} x∉ΓΔ Γ⊢b (app ΓΔ⊢m ΓΔ⊢m₁) = {!!}
thinning {Δ = Δ , y ∷ b} x∉ΓΔ Γ⊢b (conv-red ΓΔ⊢m ΓΔ⊢m₁ x) = {!!}
thinning {Δ = Δ , y ∷ b} x∉ΓΔ Γ⊢b (conv-exp ΓΔ⊢m ΓΔ⊢m₁ x) = {!!}
